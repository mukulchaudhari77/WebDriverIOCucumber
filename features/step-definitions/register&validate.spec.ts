...
Then(/^I get navigated on another page and see the Heading Text as \"([^\"]*)\"$/, async (dummyregistrationform) => {
...
Then(/^I see the Heading Text \"([^\"]*)\"$/, async (dropdown) => {
    await expect(dropDownPage.clickDropdown).toHaveText(dropdown)
});
...
When(/^I click on New Browser Tab in open new window$/, async () => {
    await windowsPage.clickOnNewBrowserTab.click()
});

Then(/^I should get navigated to new Tab and see text \"([^\"]*)\"$/, async (newbrowsertab) => {
